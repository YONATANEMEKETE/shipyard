 ...
 *
 * 1. No token → invalid-link notice with a way back to forgot-password
 * 2. Token present → new-password form (main variant)
 * 3. After a successful (stubbed) submit → updated confirmation
 */
  ...
  if (!updated) {
    return (
      <div className="flex flex-col gap-8">
        ...
        <ResetPasswordForm token={token} onUpdated={() => setUpdated(true)} />
      </div>
    );
  }
