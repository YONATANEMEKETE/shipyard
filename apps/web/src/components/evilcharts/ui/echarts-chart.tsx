...
export type AtLeastOneThemeColor =
  { light: string[]; dark?: string[] } | { light?: string[]; dark: string[] };
...
export function distributeColors(colors: string[], maxCount: number): string[] {
  ...
  for (let i = 0; i < available; i++) {
    ...
    for (let j = 0; j < slots; j++) result.push(colors[i] ?? colors[0] ?? '');
  }
...
export function normalizeColor(value: string): string {
  ...
  const data = normalizerCtx.getImageData(0, 0, 1, 1).data;
...
export function withAlpha(color: string, alpha: number): string {
  ...
  const base = rawAlpha === undefined ? 1 : Number.parseFloat(rawAlpha) || 0;
