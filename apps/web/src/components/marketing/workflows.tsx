...
const CARDS = [
  {
    ...
    body: 'A project is a page for one objective: the issues inside it, an owner, and a lifecycle of its own — planned, active, completed — without a second tool to keep in step.',
    visual: <ProjectsVisual />,
  },
  {
    eyebrow: 'Run',
    heading: 'Cycles that keep the pace honest',
    body: 'Time-boxed cycles with progress that comes from the work itself — nothing to update by hand, nothing to reconcile.',
    visual: <CycleVisual />,
  },
  {
    eyebrow: 'Collaborate',
  ...
  return (
    <section className="border-b border-ds-border">
      {/* The top of this section sits close under the hero's product band: the
          ...
          a third of the bottom's — the panel reads as the next thing on the page
      ...
      <Container className="pt-10 pb-20 sm:pt-12 sm:pb-28">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-3">
          {CARDS.map((card) => (
            <div
              key={card.eyebrow}
              className="flex flex-col rounded-xl border border-ds-border bg-ds-surface p-6"
            >
              <p className="font-mono text-[10px] font-semibold tracking-[1.2px] text-ds-text-muted uppercase">
                {card.eyebrow}
              </p>
              ...
              <div className="mt-6 rounded-lg border border-ds-border bg-ds-bg p-4">
                {card.visual}
              </div>
            </div>
          ))}
