...
import { useLeaveWorkspace, useMembers, useTransferOwnership } from '@/hooks/use-members';
...
export function LeaveWorkspaceDialog({ slug, workspaceName, workspaceRole, open, onOpenChange }: LeaveWorkspaceDialogProps) {
  return (
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        {workspaceRole === 'OWNER' ? (
          <LeaveOwnerContent key={open ? 'open-owner' : 'closed-owner'} slug={slug} workspaceName={workspaceName} onOpenChange={onOpenChange} />
        ) : (
          <LeaveMemberContent key={open ? 'open-member' : 'closed-member'} slug={slug} workspaceName={workspaceName} workspaceRole={workspaceRole} onOpenChange={onOpenChange} />
        )}
      </DialogPrimitive.Portal>
...
function LeaveMemberContent({
  ...
  const leaveMutation = useLeaveWorkspace(slug, {
    ...
    onError: (error) => {
      showToast({ status: 'error', title: 'Failed to leave workspace', description: error.message });
    },
  });
    ...
    <>
      ...
      >
        ...
        <div className="flex w-full items-center gap-3.5">
          <span aria-hidden className="grid size-11 shrink-0 place-items-center rounded-lg border border-[#EDD9A8] bg-ds-warning-soft">
          ...
          <DialogPrimitive.Close asChild>
            <button type="button" aria-label="Close" className="grid size-8 shrink-0 place-items-center rounded-lg border border-ds-border bg-ds-bg text-muted-foreground transition-colors hover:text-foreground">
        ...
        <p className="text-[13px] leading-[1.6] text-muted-foreground">
          You&apos;ll lose access to this workspace immediately. Any projects you own transfer automatically to the
          Workspace Owner.
        </p>
        ...
        <div className="flex w-full items-center gap-2 rounded-lg border border-[#EDD9A8] bg-ds-warning-soft px-3 py-2.5">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-0.5">
            <span className="text-xs font-semibold leading-none text-foreground">Projects will transfer to Owner if any</span>
            <span className="text-[10.5px] leading-none text-muted-foreground">Ownership moves to the Workspace Owner — you can&apos;t choose who.</span>
          </div>
        </div>

        <div className="flex w-full items-center justify-end gap-2.5">
          <Button type="button" variant="ghost" onClick={() => onOpenChange(false)} disabled={busy} className="h-9 gap-2 rounded-md px-4 text-sm font-medium text-muted-foreground hover:text-foreground">
            <X className="size-4" aria-hidden />
            Cancel
          </Button>
          <Button type="button" variant="destructive" onClick={() => !busy && leaveMutation.mutate()} disabled={busy} className="h-9 gap-2 rounded-md px-4 text-sm font-semibold">
            {busy ? <span className="size-4 animate-spin rounded-full border-2 border-white/30 border-t-white" /> : <LogOut className="size-4" aria-hidden />}
...
function LeaveOwnerContent({
  ...
  const eligible = useMemo(() => {
    ...
    // owner can transfer to Admin or Member, not to self and not to Owner (there is only one owner)
    return members.filter((m) => m.role !== 'OWNER' && m.userId !== selfId);
  }, [members, session?.user.id]);

  const [targetId, setTargetId] = useState<string | undefined>(() => eligible[0]?.id);
  ...
  const transferMutation = useTransferOwnership(slug, {
    ...
    onError: (error) => {
      showToast({ status: 'error', title: 'Failed to transfer ownership', description: error.message });
    },
  });

  const busy = transferMutation.isPending;
  const selected = eligible.find((m) => m.id === targetId);

  const confirm = () => {
    if (!targetId || busy) return;
    transferMutation.mutate({ targetMemberId: targetId });
  };
  ...
  return (
    <>
      ...
      >
        ...
        <p className="text-[13px] leading-[1.6] text-muted-foreground">
          To leave this workspace you must first transfer ownership to another Member or Admin. You&apos;ll become an
          Admin, and the selected person becomes the new Owner. This can&apos;t be undone.
        </p>

        <div className="flex w-full flex-col gap-1.5">
          <span className="text-[11px] font-semibold text-foreground">Transfer ownership to</span>
          {eligible.length === 0 ? (
              ...
              <SelectTrigger className="h-10 rounded-[10px] border-ds-border bg-ds-surface px-3 text-[12px]">
                <span className="flex items-center gap-2">
                  {selected?.image ? (
                    ...
                    <img src={selected.image} alt="" className="size-6 rounded-full border object-cover" />
                  ) : (
                  ...
                  <span className={cn(selected ? 'text-foreground' : 'text-muted-foreground')}>
                    {selected ? `${selected.name} · ${selected.role}` : 'Select member'}
                  </span>
                </span>
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="flex w-full items-center gap-2 rounded-lg border border-[#EDD9A8] bg-ds-warning-soft px-3 py-2.5">
          <ShieldAlert className="size-[13px] shrink-0 text-ds-warning" aria-hidden />
          <p className="text-[11px] leading-[1.5] text-foreground">Both people&apos;s roles update at once. Projects you own move with you to the new Owner.</p>
        </div>
